...
import { batch, For, onMount, type VoidComponent } from "solid-js";
...
export const BoardExample = () => {
	...
	const closestEntity: CollisionDetector = (
		draggable,
		droppables,
		context
	) => {
			...
			if (changingGroup) {
				const belowLastItem =
					...
					draggable.transformed.center.y >
						closestItem.transformed.center.y;
			...
			>
				...
				<DragOverlay>
					{(draggable) => {
						...
						return !entity ? null : draggable &&
						  isSortableGroup(draggable) ? (
								<GroupOverlay
									name={entity.name}
